        ...
        <header
            className={cn(
                ...
                (role === 'admin' || role === 'seller')
                    ? "fixed top-0 left-0 right-0 z-[200] px-4 max-md:[padding-top:max(env(safe-area-inset-top),_12px)] max-md:pb-2 md:sticky md:top-0 md:h-14 md:px-4"
                    : "fixed top-0 left-64 right-0 h-14 px-4 z-40"
            )}
        >
                ...
                <div className="flex items-center space-x-2 mr-4 md:hidden">
                    {logoUrl ? (
                        <div className="h-8 w-8 rounded-lg overflow-hidden shadow-md shadow-primary/10 border border-gray-100">
                            <img loading="lazy" src={logoUrl} alt={appName} className="h-full w-full object-cover" />
                        </div>
                    ) : (
