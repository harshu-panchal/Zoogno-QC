...
// The actual card markup (image, price, quantity stepper, animations) lives in
// this inner, memoized component. It receives only the primitives/stable
// callbacks it needs as props, so React.memo's shallow comparison can
// correctly skip re-rendering a card whose own product/quantity/wishlist
// state hasn't changed — even though the outer ProductCard below still
// re-runs (cheaply) on every cart/wishlist context update. Previously the
// entire card tree (images, framer-motion elements, event handlers) lived
// directly inside the component subscribed to cart context, so adding one
// item to the cart re-rendered every visible card, not just the one that
// changed.
const ProductCardView = React.memo(function ProductCardView({
  product,
  badge,
  className,
  compact,
    ...
    <div
      ...
      <div className="relative">
        ...
        <div
          ...
          <img
            ref={imageRef}
            src={applyCloudinaryTransform(product.image || product.mainImage, "f_auto,q_auto,w_400,dpr_auto")}
            alt={product.name}
            loading="lazy"
...
const ProductCard = React.memo(
  ({ product, badge, className, compact = false, neutralBg = false }) => {
    ...
    const handleCloseVariantSheet = React.useCallback(() => setIsVariantSheetOpen(false), []);
    ...
    return (
      <ProductCardView
        ...
        isVariantSheetOpen={isVariantSheetOpen}
        onCloseVariantSheet={handleCloseVariantSheet}
        initialVariantSkuForSheet={initialVariantSkuForSheet}
