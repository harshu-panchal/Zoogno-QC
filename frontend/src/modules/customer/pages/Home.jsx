...
const Home = () => {
  ...
  const fetchData = async ({ forceRefresh = false } = {}) => {
    ...
    try {
      ...
      if (prodRes.data.success) {
        ...
        nextHomeData.products = dbProds.map((p) => ({ ...p, id: p._id, image: p.mainImage || p.image || "https://images.unsplash.com/photo-1550989460-0adf9ea622e2?auto=format&fit=crop&q=80&w=400&h=400", price: p.salePrice || p.price, originalPrice: p.price, weight: p.weight || "1 unit", deliveryTime: p.sellerId?.estimatedDeliveryTime || "8-15 mins", variants: p.variants || [], shopName: p.sellerId?.shopName || p.shopName || "Unknown" }));
      }
  ...
  const hydrateSelectedSectionProducts = async (sections = []) => {
    ...
    try {
      ...
      const fetchedMissing = missingResults.filter((r) => r.status === "fulfilled").flatMap((r) => { const p = r.value?.data?.result || r.value?.data?.results; return Array.isArray(p) ? p : (p ? [p] : []); }).map((p) => ({ ...p, id: p._id, image: p.mainImage || p.image || "https://images.unsplash.com/photo-1550989460-0adf9ea622e2?auto=format&fit=crop&q=80&w=400&h=400", price: p.salePrice || p.price, originalPrice: p.price, weight: p.weight || "1 unit", deliveryTime: p.sellerId?.estimatedDeliveryTime || "8-15 mins", variants: p.variants || [], shopName: p.sellerId?.shopName || p.shopName || "Unknown" }));
            ...
            <div className="relative w-full overflow-hidden">
              {heroConfig.mediaType === "dynamic" && heroConfig.dynamicConfig ? (
                ...
                <div className="w-full relative overflow-hidden shadow-sm flex items-center justify-center">
                  <img loading="lazy" src={CardBanner} alt="Banner" className="w-full h-full object-cover object-center aspect-[16/8] sm:aspect-[21/9]" />
                </div>
              )}
            </div>
          </motion.div>
