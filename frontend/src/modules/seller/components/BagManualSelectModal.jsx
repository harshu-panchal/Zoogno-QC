...
const BagManualSelectModal = ({ isOpen, onClose, onSelect, currentBagId }) => {
    ...
    const fetchAvailableBags = async (search = '') => {
        setLoading(true);
        try {
            const params = { status: 'assigned', limit: 50 };
            if (search) params.search = search;

            const res = await sellerApi.getMyBags(params);
            setBags(res.data?.data || []);
        } catch (error) {
            console.error('Failed to fetch available bags:', error);
            showToast('Failed to load available bags', 'error');
        } finally {
            setLoading(false);
        }
    };
    ...
    const handleSelect = (bagId) => {
        if (bagId === currentBagId) {
            showToast('This bag is already linked to the current order', 'warning');
            return;
        }
        onSelect(bagId);
    };
            ...
            >
                <div className="px-5 py-4 border-b border-slate-100 flex items-center justify-between bg-slate-50/50">
                    <div className="flex items-center gap-3">
                        ...
                        <div>
                            <h3 className="text-sm font-black text-slate-900">Select Available Bag</h3>
                ...
                <div className="flex-1 overflow-y-auto p-4 bg-slate-50">
                    {loading ? (
                        ...
                        <div className="flex flex-col items-center justify-center py-10 text-center">
                            ...
                            <p className="text-sm font-bold text-slate-800">No bags found</p>
                            <p className="text-xs text-slate-500 font-medium mt-1 max-w-[200px]">
                                {searchTerm ? "Try adjusting your search criteria." : "You don't have any bags available for assignment."}
                            </p>
                        </div>
                    ) : (
...
export default BagManualSelectModal;
